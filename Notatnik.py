...
class Notatnik():
    ...
    def wyniki2(self):
        ...
        for record in lista2:
            recordbutton = Button(self.newWindow4, text=record[1], bg="gray80", font=('Calibri Light', 15), command=lambda record=record: self.pokazNotke(record[2]))
    ...
    def wyniki(self):
        ...
        for record in lista:
            recordbutton = Button(self.newWindow2, text=record[1] +"|" +record[0], bg="gray80", font=('Calibri Light', 15), command=lambda: self.pokazNotke(record[2]))
            recordbutton.pack(fill=X)
